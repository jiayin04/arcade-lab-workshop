...
export class Landing implements AfterViewInit, OnDestroy {
  ...
  private initThree(): void {
    ...
    this.camera = new THREE.PerspectiveCamera(45, w / h, 0.1, 0);
  ...
  private buildLaptop(): void {
    ...
    const deck = new THREE.Mesh(
      new THREE.BoxGeometry(2.8, 0.01, 1.4),
    ...
    const tp = new THREE.Mesh(
      new THREE.BoxGeometry(0.85, 0.012, 0.58),
    ...
    const edge = new THREE.Mesh(
      new THREE.BoxGeometry(3.22, 0.12, 2.12),
    ...
    const bezel = new THREE.Mesh(
      new THREE.BoxGeometry(2.95, 0.065, 1.86),
      new THREE.MeshStandardMaterial({ color: 0x0a0a0a, roughness: 0.15, metalness: 0.95 })
    );
    ...
    this.lid.rotation.x = -Math.PI * 0.5;
  ...
  private animate(): void {
    ...
    this.laptopGroup.rotation.y = Math.sin(this.floatT * 0.4) * 0.15;
    this.laptopGroup.position.y = -0.25 - Math.sin(this.floatT * 0.6) * 0.06;
  ...
  private updateLaptopColors(): void {
    ...
    this.rimLight.color.set(isRetro ? 0x00ffaa : 0x60a5fa);
    this.screenMat.color.set(isRetro ? 0x0d0221 : 0x020c1b);
    this.screenMat.emissive.set(isRetro ? 0xc026d3 : 0x2563eb);
